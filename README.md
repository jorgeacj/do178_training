...
- [x] Write “Tailored DO-178C approach for this exercise” (lifecycle + objectives mapping)
...
- [x] Implement BSP wrapper: TIM2 PWM (servo output on PA0, CH1)
- [x] Implement BSP wrapper: ADC1 (servo position feedback on PF11 / INP2)
- [x] Implement BSP wrapper: UART4 TX/RX (PC10/PC11, 115200 baud)
- [ ] Implement remaining BSP wrappers: GPIO, CAN, watchdog, EXTI
- [ ] Define and document ISR policy (minimal ISR, defer work, shared data rules)
- [x] Set up FreeRTOS scheduler with two static tasks (defaultTask + servoTask @ 10 ms)
- [x] Implement dual-core (CM7/CM4) boot synchronization via hardware semaphore

### Timing / determinism

- [x] Implement periodic execution model (FreeRTOS osDelay-based, 10 ms servo loop)
...
### Servo / flap control (PWM + ADC feedback)

- [x] Implement PWM servo driver (`pwm_servo.c/.h`) — start, set-angle, set-value, step-toward-angle
- [x] Implement ADC feedback subsystem — init (calibration + start), read (poll + compute), compute-angle
...
- [x] Create Simulink model for flap state machine (Idle → Taxing → Take-off → Cruise → Landing)
- [x] Generate Embedded Coder C code (`flap.c/.h`) targeting ARM Cortex-M
- [x] Integrate Simulink-generated PID controller with servo PWM output
- [x] Wire ADC feedback angle into Simulink model input (`rtU.FeedbackServo`)
- [ ] Document model-to-code traceability (Simulink requirements ↔ generated code ↔ test cases)
- [ ] Freeze model baseline version and archive generated code as CI artifact
...
- [x] Implement step-toward-angle smooth motion with configurable step size
...
- [x] Write MCDC intro note with worked example (Annex C in `README.md`)
